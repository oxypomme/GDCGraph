...
type PlayerListType = { data: IPlayersType[]; updated: string };
...
export const fetchPlayerList = createAsyncThunk(
	...
	async () => await (await fetch(urljoin(urlAPI, "/api/players"))).json()
);
export const fetchPlayer = createAsyncThunk(
	...
	async (id: string, { getState }) => {
		...
		const currIndex = currPlayers.findIndex(
			(val) => val.id === parseInt(id) || val.name === id
		);
		if (currIndex !== -1) {
		...
		const player = await (
			await fetch(urljoin(urlAPI, `/graph/players/${id}`))
		).json();
...
export const playerSlice = createSlice({
	...
	extraReducers: {
		...
		[fetchPlayerList.fulfilled.type]: (
			state,
			{ payload: playerList }: PayloadAction<PlayerListType>
		): PlayerState => ({
			...state,
			playerList,
			isFetching: false,
		...
		[fetchPlayer.fulfilled.type]: (
			state,
			{ payload: player }: PayloadAction<IPlayerType | null>
		): PlayerState => {
...
export const selectPlayer = createSelector(
	getState,
	(state) => state.players[state.lastFetchIndex]
);
export const selectPlayerFetching = createSelector(
	getState,
