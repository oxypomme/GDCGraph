...
const AllPlayers = (): JSX.Element => {
	...
	const getTop3Stats = (): unknown[] => {
		...
		if (playerList.data.length > 0) {
			return [
				...
				...playerList.data
					.slice()
					.sort((a, b) => b.count_missions - a.count_missions)
					.slice(0, 3)
					.map((p) => [p.name, p.count_missions]),
			];
		}
		return [headers, ["", 0]];
	};
	...
	const getAverage = (): number => {
		if (playerList.data.length > 0) {
			...
			return sumMiss / playerList.data.length;
		}
		return 0;
	};
	...
	return (
		<div>
			...
			<UpdateLabel>
				Mis à jour le :{" "}
				{dayjs(playerList?.updated).format("DD/MM/YYYY - HH:mm:ss")}
			</UpdateLabel>
			<h2>{playerList?.data.length} joueurs ont été trouvés</h2>
			<Container>
				<Chart
					width={"auto"}
					height={"auto"}
			...
			<p>
				...
				{averagePlayer?.toLocaleString(undefined, { maximumFractionDigits: 0 })}{" "}
				missions en moyenne
			</p>
		</div>
	);
